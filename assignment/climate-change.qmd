...
The overall trend in the CO2 data shows a steady and significant increase in atmospheric CO2 concentrations over time. There is a positive linear correlation.
...
```{r}
ggplot(co2, aes(x = month, y = monthly_average, group = year)) +
  geom_line() +
  stat_summary(fun = mean, geom = "line", color = "red") +
  labs(x = "Month", y = "CO2 concentration (ppm)", title = "Variation of CO2 Levels")
```
The highest levels are in May and the lowest are around September. The CO2 levels most likely vary because of photosynthetic rates. When plants release CO2 in the fall and winter, they steadily increase. 
...
The columns are the calendar year and the difference in temperature from the norm. 
Global temp is the temperature of the the planet and global temperature anomaly is the difference from the mean temperature based on trends.
...
Based on the plot, the temperatures have increased at a fast rate. This graph shows the global warming trends.
...
**Answer 6:**
I looked up how to calculate rolling averages in R studio tidyverse using ChatGPT. It told me to employ the zoo package and make minor changes based on the structure of the table. 
...
**Answer 7:**
The annual average is the temperature anomaly for each year, the 5-year average is the temperature anomaly over a 5 year span. 
```{r}
global_temp <- global_temp %>%
  mutate(rolling_avg_5 = rollmean(No_Smoothing, k = 5, fill = NA),
...
ggplot(global_temp, aes(x = Year)) +
  geom_line(aes(y = No_Smoothing), color = "red") +
  geom_line(aes(y = rolling_avg_5), color = "orange") +
  geom_line(aes(y = rolling_avg_10), color = "green") +
...
The variation on the of the data gets less and less as the rolling average gets to a higher year span. The variation gets lower because you are averaging more amounts of data together.
**Question 9:**

By examining the data here, what evidence do you find or not find for such a pause? 

**Answer 9:**
There is a temporary slowing but an overall warming trend seen by the 10-year and 20-year showing that there is not a pause. 
...
ggplot(vostok_co2, aes(x = Age_Air_BP, y = CO2_ppm)) +
  ...
  labs(x = "Years Before Present", y = "CO₂ Concentration (ppm)", 
       title = "CO₂ Concentrations from Ice Core Data (Vostok, Antarctica)")
...
ggplot(co2_combined, aes(x = year, y = co2_ppm)) +
  geom_line(color = "blue") +
  labs(x = "Year", y = "CO2 Concentration (ppm)", 
       title = "CO₂ Concentrations Over Time: Ice Core vs. Modern")

```
The data is historical CO2 concentrations from Vostok ice. The columns are is the depth in meters, the age of the ice sample in years before present, the age of the air sample in years before present, and CO2 concentration in parts per million. The data comes from glacial ice samples in Vostok, Antartica. 

The earlier years have lower resolution and the more recent years have better resolution. There are more gaps in the earlier data. 

The rate of change is very extreme as you can see from the plots. The conclusion from this is that the massive spike in CO2 levels is human-related with the higher increase closer to the present. 
